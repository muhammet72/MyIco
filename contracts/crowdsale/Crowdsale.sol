...
contract Crowdsale is Context, ReentrancyGuard {
  ...
  /**
   * @param _initialRate Number of token units a buyer gets per wei
   ...
   **/
  constructor(uint256 _initialRate, address payable walletAddress, IERC20 _tokenAddress) {
  ...
  function buyTokens(address beneficiary) public payable nonReentrant {
    ...
    // update state
    (bool success, uint256 result) = _weiRaised.tryAdd(weiAmount);
    require(success);
    _weiRaised = result;
  ...
  function _getTokenAmount(uint256 weiAmount) internal view virtual returns (uint256) {
    (bool success, uint256 result) = weiAmount.tryMul(_rate);
    require(success);
    return result;
  }
